...
class MavController:
    ...
    def pose_callback(self, data):
        ...
        if self.landCommandNotGiven:
            e_x = current_x - self.all_waypoints[self.next_wp][0]
            e_y = current_y - self.all_waypoints[self.next_wp][1]
        #e_z = current_z - self.all_waypoints[self.next_wp][2]

        print([current_x, current_y, current_z])
        print(self.all_waypoints[self.next_wp])
        print("\n")

        if np.sqrt(e_x*e_x + e_y*e_y) < 0.15:
            #print([current_x, current_y, current_z])
            print(['In the well:', current_x, current_y, current_z])
            self.set_vel(0, 0, 0)
            if self.next_wp < len and self.landCommandNotGiven:
                self.next_wp += 1
                print('WAYPOINT-TRANSITION')
                self.goto_xyz()
            else:
                print('LANDING')
                self.landCommandNotGiven = False
                self.land()
